...
impl<'db, 'sess> VdBsqElaboratorInner<'db, 'sess> {
    pub(super) fn transcribe_product_data_and_ty(
        &self,
    ...
    pub(super) fn transcribe_product_stem_and_coeff_data_and_ty(
        &self,
        stem: VdBsqProductStem<'sess>,
        ...
        if coeff.is_one() {
            match stem {
                VdBsqProductStem::Atom(atom) => self.transcribe_factors_data_and_ty(
                    [Left(coeff), Right(Left(atom))],
                    hypothesis_constructor,
                ),
                VdBsqProductStem::NonTrivial(stem) => todo!(),
            }
        } else {
            todo!()
        }
    ...
    pub(super) fn transcribe_factors_data_and_ty(
        &self,
        factors: impl IntoIterator<
            Item = Either<
                VdBsqLitnumTerm<'sess>,
                Either<VdBsqAtomTerm<'sess>, (VdBsqNumTerm<'sess>, VdBsqNumTerm<'sess>)>,
            >,
        >,
        hypothesis_constructor: &mut VdMirHypothesisConstructor<'db, VdBsqHypothesisIdx<'sess>>,
    ) -> (VdMirExprData, VdType) {
        ...
        let (leader_data, leader_ty) =
            self.transcribe_factor_data_and_ty(leader, hypothesis_constructor);
        let fst_follower = factors.next().unwrap();
        let (fst_follower_data, fst_follower_ty) =
            self.transcribe_factor_data_and_ty(fst_follower, hypothesis_constructor);
        let fst_signature = self.mul_signature(leader_ty, fst_follower_ty);
        ...
        for factor in factors {
            let (follower_data, follower_ty) =
                self.transcribe_factor_data_and_ty(factor, hypothesis_constructor);
            let signature = self.mul_signature(acc_ty, follower_ty);
    ...
    fn transcribe_factor_data_and_ty(
        &self,
        factor: Either<
            VdBsqLitnumTerm<'sess>,
            Either<VdBsqAtomTerm<'sess>, (VdBsqNumTerm<'sess>, VdBsqNumTerm<'sess>)>,
        ...
        match factor {
            Left(litnum) => self.transcribe_litnum_data_and_ty(litnum, hypothesis_constructor),
            Right(atom_or_exponential) => match atom_or_exponential {
                Left(atom) => self.transcribe_atom_data_and_ty(atom, hypothesis_constructor),
                Right((base, exponent)) => {
                    self.transcribe_exponential_data_and_ty(base, exponent, hypothesis_constructor)
                }
            },
        }
    }

    fn transcribe_exponential_data_and_ty(
        &self,
        base: VdBsqNumTerm<'sess>,
        exponent: VdBsqNumTerm<'sess>,
        hypothesis_constructor: &mut VdMirHypothesisConstructor<'db, VdBsqHypothesisIdx<'sess>>,
