use super::*;
use crate::{
    default_table::VdDefaultGlobalDispatchTable,
    dispatch::{
...
use visored_term::{term::VdTermData, ty::VdType};
...
impl VdDefaultGlobalDispatchTable {
    pub fn infer_equivalence_signature(
        ...
        let separator = if prev_item_ty.data() == &VdTermData::PROP {
    ...
    pub fn infer_base_prefix_opr_signature(
        ...
        match self.base_prefix_opr_default_dispatch(opr, opd_ty).unwrap() {
    ...
    pub fn infer_pow_signature(&self, base_ty: VdType, exponent_ty: VdType) -> VdPowerSignature {
        match self.power_default_dispatch(base_ty, exponent_ty).unwrap() {
    ...
    pub fn infer_base_binary_opr_signature(
        ...
        let Some(dispatch) = self.base_binary_opr_default_dispatch(lopd_ty, opr, ropd_ty) else {
    ...
    pub fn infer_base_chaining_separator_signature(
        ...
        match self.base_separator_default_dispatch(prev_item_ty, base_separator, next_item_ty) {
    ...
    pub fn infer_base_comparison_separator_signature(
        ...
        match self.base_separator_default_dispatch(
            prev_item_ty,
            base_separator.into(),
            next_item_ty,
        ) {
    ...
    pub fn infer_base_sqrt_signature(&self, radicand_ty: VdType) -> VdBaseSqrtSignature {
        match self.base_sqrt_default_dispatch(radicand_ty).unwrap() {
    ...
    pub fn infer_power_signature(&self, base_ty: VdType, exponent_ty: VdType) -> VdPowerSignature {
        let Some(dispatch) = self.power_default_dispatch(base_ty, exponent_ty) else {
